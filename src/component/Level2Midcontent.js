...
const Level2Midcontent = () => {
  ...
  //suffix array
  const suffixArray = ["ize", "ly", "est", "ful", "ish", "en"];
  ...
  useEffect(() => {
    const calculateBubblePositions = () => {
      const newBubbles = suffixArray.map((suffix, index) => {
        //console.log(index);
        const x =
          index % 2 === 0
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      context.fillText("Level-2 Suffix", xpos + 100, ypos + 25);
      ...
      if (randomWordArray.length !== 10) {
        ...
        if (score < 6) {
          ...
          Swal.fire({
            ...
            text: `Your score is ${score}`,
            icon: "success",
            confirmButtonText: "Proceed to Level-3",
          }).then((result) => {
            if (result.isConfirmed) {
              navigate("/level3");
            }
